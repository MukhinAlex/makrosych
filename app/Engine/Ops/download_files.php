...
return [
    ...
    'description' => 'Скачивает файлы по ссылкам из указанного поля и раскладывает их по папкам. Поддерживает ссылки Яндекс Диска (автоматически разрешает публичную ссылку в набор файлов) и обычные http-ссылки. Имя и папка задаются шаблоном пути.',
    'network' => true,
    'ai' => false,
    'params' => [
        ...
        'resolve' => ['type' => 'bool', 'default' => true, 'desc' => 'Разрешать публичные ссылки Яндекс Диска'],
    ...
    'handler' => static function (Context $ctx, array $p): void {
        ...
        foreach ($ctx->rows as $row) {
            ...
            foreach ($urls as $url) {
                ...
                $targets = [];
                if ($resolve && Http::isYandex($url)) {
                    if ($ctx->dryRun) {
                        $ctx->plan('file', 'публичная ссылка Яндекс Диска будет разрешена', ['url' => $url, 'group' => $group]);
                        $planned++;
                        continue;
                    }
